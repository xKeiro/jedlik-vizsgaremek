...
from ..schemas import product_category_schemas
...
@router.get('/', status_code=status.HTTP_201_CREATED, response_model=product_category_schemas.ProductCategoriesResponse)
...
@router.post('/', response_model=product_category_schemas.ProductCategoryResponse)
async def post_create_new_category(category: product_category_schemas.ProductCategoryInputPost,
...
@router.get('/{category_id}', response_model=product_schemas.ProductsResponse)
async def get_products_by_category_id(category_id: UUID4, db: Session = Depends(get_db)):
    products = db.query(models.Product).filter(models.Product.category_id == category_id,
                                               models.Product.discontinued == False).order_by(
        models.Product.title).all()
    if not products:
...
@router.patch('/{category_id}', response_model=product_category_schemas.ProductCategoryResponse)
async def patch_products_by_category_id(category_id: UUID4,
                                        category: product_category_schemas.ProductCategoryInputPatch,
                                        db: Session = Depends(get_db)):
